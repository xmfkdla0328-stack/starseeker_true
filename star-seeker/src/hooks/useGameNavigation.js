...
export default function useGameNavigation() {
  ...
  const [selectedChapterId, setSelectedChapterId] = useState('prologue');
  ...
  const actions = useMemo(() => ({
    ...
    goStoryNodeSelect: (chapterId) => {
        if (chapterId) setSelectedChapterId(chapterId);
        setGameState('story_node_select');
    }, 
  ...
  return { gameState, selectedChapterId, ...actions };
}